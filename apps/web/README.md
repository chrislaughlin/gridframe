...
- lazy SQLite-backed dashboard creation per user;
...
server/dashboard/             # Server-side logic (not a Next.js convention, just a directory)
  ...
  repository.ts               #   SqliteDashboardRepository (SQLite persistence)
  ...
  bootstrap-worker.ts         #   used in tests for concurrent bootstrap
  migrations/0001-initial.sql #   SQLite schema
...
Opens a SQLite file (or `:memory:`) via `better-sqlite3`, runs pending migrations, and returns the connection. Path is configured by `GRIDFRAME_DATABASE_PATH` (default: `.data/gridframe.sqlite`).

Schema (`migrations/0001-initial.sql`):
...
`SqliteDashboardRepository` implements both a local `DashboardRepository` interface and the `GridframeDashboardRepository` type from `@gridframe/server`. All mutations run inside SQLite transactions and use the `revision` column for optimistic locking — every write calls `incrementRevision` which atomically checks `revision = ?` and throws `DashboardRevisionConflictError` on mismatch.

### Service (`service.ts`)

Singleton factory. Calls `openDashboardDatabase` once, wraps it in `SqliteDashboardRepository`, and caches the instance for the lifetime of the server process.
...
All routes are `export const runtime = "nodejs"` because they depend on `better-sqlite3`.
...
|---|---|---|
| `GRIDFRAME_DATABASE_PATH` | `.data/gridframe.sqlite` | Path to SQLite database file |
...
Tests use `:memory:` SQLite databases and are scoped per file. Concurrent bootstrap tests use a separate CLI worker (`bootstrap-worker.ts`) spawned via `tsx`.
